...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function view_page($course, $quiz, $cm, $context, $viewobj) {
        ...
        $output .= $this->view_update_button($course, $quiz, $cm, $context, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'quizattempt');
        return $output;
    }

    /**
     * Function added by Ictineo for ESCI to add custom button to update unclosed
     * quiz, see update_attempts.php
     */
    public function view_update_button($course, $quiz, $cm, $context, $viewobj) {
      if(has_capability('mod/quiz:manage', $context)) { 
        $output = '';
        $output .= $this->box_start('updateattempts');
        $output .= $this->single_button(new moodle_url('/mod/quiz/update_attempts.php',
                      array('cmid' => $cm->id, 'quiz' => $quiz->id, 'courseid' => $course->id)), 
                      get_string('updateattempts','quiz'),
                      'get', array('class' => 'continuebutton'));
        $output .= $this->box_end();
